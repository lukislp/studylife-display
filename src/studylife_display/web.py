...
`GET /healthz` is outside that model: no cookie, no same-origin check, a JSON summary of
the panel's state for an uptime monitor. It carries no secret - never the token, never the
cookie - and never calls the StudyLife API. `GET /connect/callback` is the other route
without a cookie: the browser arriving there comes from StudyLife's redirect (which never
carries a SameSite=Strict cookie); the single-use state generated by an authenticated
request is what binds it to this display.
"""
...
from studylife_display.config import LANGUAGES, ROTATIONS, Settings, WebOverrides
